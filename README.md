...
**UAL MailItemsAccessed × Microsoft Purview Content Search Correlation for BEC investigations**
...
- **UAL export processing** — loads pre-filtered CSV or XLSX UAL exports and auto-detects column names
- **JSON payload parsing** — walks the `AuditData → Folders → FolderItems → InternetMessageId` structure across every event row
- **Session & user filtering** — optionally scope extraction to a specific TA session token or UserId
- **Purview Content Search correlation** — loads `Items.csv` from a Purview export and matches IDs against the UAL-extracted set
- **Normalised matching** — case-insensitive comparison with angle bracket stripping to handle ID format variance
- **Real-time diagnostics** — columns detected, row counts, REST-only event counts, parse errors, and unique IDs extracted
- **Styled XLSX export** — output workbook with All Items, TA Accessed, and UAL Message IDs sheets; matched rows highlighted green
- **Entirely offline** — no network calls, no external data transmission
...
This tool is intended for use by authorised security professionals conducting legitimate incident response and forensic investigations. Ensure all M365 queries are performed in accordance with applicable laws and your organisation's policies.
